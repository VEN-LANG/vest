...
export abstract class Model {
  ...
  protected _changes: ModelAttributes = {};
    ...
    const proxy = new Proxy(this, {
      get: (target: any, prop: PropertyKey, receiver: any) => {
        if (typeof prop === "string") {
          ...
          // Use studly-case conversion to match buildMethodName (e.g. full_name → getFullNameAttribute)
          const studlyProp = prop.split("_").filter(Boolean).map((s: string) => s.charAt(0).toUpperCase() + s.slice(1)).join("");
          const hasAccessor =
              target.hasAccessor?.(prop) ||
              typeof target[`get${studlyProp}Attribute`] === "function";
  ...
  static withoutGlobalScope<T extends typeof Model>(
    ...
    this.ensureBooted();
    const builder = new EloquentBuilder<InstanceType<T>>(this as any);
    // Apply soft delete filter if enabled (without going through applyScopes)
    if ((this as any).softDeletes) {
      builder.whereNull("deleted_at");
      (builder as any).appliedSoftDeleteFilter = true;
    }
    // Apply only global scopes not in the exclusion list
    Object.entries((this as any).globalScopes || {}).forEach(([name, scope]) => {
      if (!scopes.includes(name)) {
        (scope as any)(builder);
      }
    });
    return builder;
  }
  ...
  getAttribute<T = any>(key: string, onlyAppended: boolean = false): T {
    ...
    try {
      ...
      // Fallback to instance method
        ...
        const accessorCtx = this.createAccessorContext();
        value = (this as any)[accessorKey].call(accessorCtx, value);
    ...
    Object.keys(attributes).forEach((key) => {
      if (staticClass.fillable.length === 0 || staticClass.fillable.includes(key)) {
        if (!staticClass.guarded.includes(key)) {
          if (!this.setAttributeWithMutator(key, attributes[key])) {
            this.setAttribute(key, attributes[key]);
          }
        }
      }
