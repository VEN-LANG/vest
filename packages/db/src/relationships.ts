  ...
  async getResults(): Promise<T[]> {
    ...
    if (getDbType() === "mongodb") {
      ...
      if (/^[0-9a-fA-F]{24}$/.test(String(localValue))) {
        try {
          firstMatchVals.push(new ObjectId(String(localValue)));
        } catch {
          /**/
        }
      }
      const throughDocs = await tc
        .find({ [this.firstKey]: { $in: firstMatchVals } } as any)
        .toArray();
      ...
      const docs = await rc.find({ [this.secondKey]: { $in: matchVals } } as any).toArray();
