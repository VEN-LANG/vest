...
```typescript
container.when(DatabaseService).needs(CacheDriver).give(() => new RedisCache())
```
  ...
  register() {
    ...
    this.app.singleton(UserService, () => {
      return new UserService(
        this.app.make(DatabaseService),
        this.app.make(CacheManager)
      )
    })

    this.app.singleton(AuthService, () => {
      return new AuthService(
        this.app.make(UserService),
        this.app.make(JwtService)
      )
    })
  }
}
