    ...
    while (this.running && !this.shouldQuit) {
      ...
      if (this.idleTicks % Worker.IDLE_CHECK_INTERVAL === 0) {
        if (this.memoryExceeded()) {
          ...
          console.log(
            `[Worker] Memory limit reached (${memMb} MB / ${this.options.memory} MB) — attempting GC recovery...`,
          );
          ...
          if (!this.memoryExceeded()) {
            ...
            console.log(
              "[Worker] Memory still exceeded after GC — stopping for supervisor restart.",
            );
            this.stop();
            break;
          }
